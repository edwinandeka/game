...
function toast(message, duration, className, completeCallback) {
    ...
    // Get container
    var $container = $('#jq-toast-container');

    // Create jq- container if it does not exist
    if (!$container.length)
        $container = $('<div id="jq-toast-container">').appendTo('body');

    // If no message, no toast
    if (!settings.message || $.isPlainObject(settings.message)) return false;
    ...
    var counterInterval = setInterval(function () {
        if (!$toast.parent().length) clearInterval(counterInterval);

        // If toast is not being dragged, decrease its time remaining
        if (!$toast.is('.panning')) settings.displayLength -= 20;
    ...
    function createToast(html) {
        ...
        var $toast = $(
            '<div class="jq-toast ' +
                settings.className +
                '" style="top: 35px; opacity: 0">'
        ).append(html);

        // Bind hammer
        if ($.isFunction(Hammer)) {
            var activationDistance = 80;
            var hammerHandler = new Hammer($toast[0], {
            ...
            hammerHandler.on('pan', function (event) {
                ...
                var opacityPercent =
                    1 - Math.abs(event.deltaX / activationDistance);
                if (opacityPercent < 0) opacityPercent = 0;

                $toast.animate(
                    { left: event.deltaX, opacity: opacityPercent },
            ...
            hammerHandler.on('panend', function (event) {
                ...
                if (Math.abs(event.deltaX) > activationDistance) {
                    ...
                    $toast.removeClass('panning').animate(
                        { left: 0, opacity: 1 },
                        {
                            duration: 300,
                            queue: false,
